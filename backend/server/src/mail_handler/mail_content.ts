...
function getProfileContent(profile: Profile) {
    ...
    content += `<li>Webbplats: <a style="color:#c6a230;" target="_blank" href="${profile.homepage}">${profile.homepage}</a></li>`
    content += '</ul>';
    content += '<br>';
    return content
}
...
function getEntryContent(entries: Entry[]) {
    ...
    entries.forEach(entry => {
        ...
        content += `<li>Illustratör/fotograf: ${entry.illustrator}</li>`
        content += `<li>Projektledare: ${entry.leader}</li>`
        content += `<li>Uppdragsgivare: ${entry.customer}</li>`
        content += `<li>Webbplats: ${entry.webpage}</li>`
        content += `<li>Omfång: ${entry.format}</li>`
        content += `<li>Storlek: ${entry.size}</li>`
        content += `<li>Bifogad fil: <a target="_blank" href="${SITE_URL}/admin/media/${entry.source}">${entry.source}</a></li>`
    })   
    return content
}